...
struct VirtualDesktopManagerVtable {
    ...
    is_window_on_current_virtual_desktop:
        unsafe extern "system" fn(*mut VirtualDesktopManager, Hwnd, *mut Bool) -> Hresult,
    get_window_desktop_id:
        unsafe extern "system" fn(*mut VirtualDesktopManager, Hwnd, *mut Guid) -> Hresult,
    move_window_to_desktop:
        unsafe extern "system" fn(*mut VirtualDesktopManager, Hwnd, *const Guid) -> Hresult,
}
...
struct ComVirtualDesktopManager {
    pointer: *mut VirtualDesktopManager,
...
impl ComVirtualDesktopManager {
    ...
    fn describe_window(&self, hwnd: Hwnd) -> (Option<String>, Option<bool>) {
        ...
        let id_result =
            unsafe { (vtable.get_window_desktop_id)(self.pointer, hwnd, &mut desktop_id) };
...
pub fn discover() -> Result<DesktopSnapshot, String> {
    ...
    for process in processes.values() {
        if is_known_background_app(&process.exe_name) {
            by_pid.entry(process.pid).or_default();
        }
    }
...
fn enumerate_displays() -> Result<Vec<DisplayInfo>, String> {
    ...
    let result =
        unsafe { EnumDisplayMonitors(ptr::null_mut(), ptr::null(), Some(enum_monitor), data) };
    if result == 0 {
...
fn annotate_display_relations(displays: &mut [DisplayInfo]) {
    ...
    for display in displays {
        display.relation_to_primary =
            display_relation(display.bounds, primary_bounds, display.is_primary);
    }
}
...
fn enumerate_windows() -> Result<Vec<RawWindow>, String> {
    ...
    if result == 0 {
        return Err("failed to enumerate Windows top-level windows".to_owned());
    }
    Ok(context.windows)
}
...
fn build_window_info(
    ...
    virtual_desktops: Option<&ComVirtualDesktopManager>,
) -> Option<WindowInfo> {
    ...
    let display = device
        ...
        .and_then(|device| {
            displays
                .iter()
                .find(|display| display.device_name == device)
        })
        .or_else(|| displays.first())?;
    let geometry_bounds = if window.minimized {
        window.restore_bounds.unwrap_or(window.bounds)
    } else {
        window.bounds
    };
...
fn build_launch_spec(
    ...
    if let Some(application_id) = app_user_model_id {
        return Some(LaunchSpec {
            strategy: LaunchStrategy::AppUserModelId,
            target: application_id.to_owned(),
        });
    }
    executable_path.map(|path| LaunchSpec {
...
mod tests {
    ...
    fn launcher_prefers_app_user_model_id() {
        let launch = build_launch_spec(
            Some("Contoso.App_123!App"),
            Some(r"C:\\Program Files\\Contoso\\app.exe"),
        )
        .expect("launch spec");
    ...
    fn launcher_falls_back_to_executable_path() {
        let launch = build_launch_spec(None, Some(r"C:\\Program Files\\Contoso\\app.exe"))
            .expect("launch spec");
