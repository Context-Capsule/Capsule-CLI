...
fn enrich_exact_powershell_locations(snapshot: &mut TerminalSnapshot, stage: &str) {
    for session in &mut snapshot.sessions {
        ...
        // WorkingDirectorySource predates direct PowerShell runspace probing and
        // currently has no dedicated Runspace variant. WindowsTerminalState is
        // the existing serialized 'trusted shell location' channel understood by
        // restore matching. Mark the directly queried runspace path trusted here;
        // the terminal.log entry below records its true provenance explicitly.
