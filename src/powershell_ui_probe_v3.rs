...
fn working_directories_windows(snapshot: &TerminalSnapshot) -> HashMap<u32, String> {
    ...
    logging::info(
        ...
        format!(
            "PowerShell UI CWD probe v3 begin: live_windows_terminal_sessions={}",
    ...
    if live_windows_terminal.is_empty() {
        logging::info(
            TERMINAL_LOG_COMPONENT,
            "PowerShell UI CWD probe v3 skipped: no live Windows Terminal sessions",
        );
        return HashMap::new();
    }
    ...
    if let Some(session) = live_windows_terminal.iter().find(|session| {
        ...
        logging::info(
            TERMINAL_LOG_COMPONENT,
            format!(
                "PowerShell UI CWD probe v3 skipped: Windows Terminal contains non-PowerShell shell={} pid={:?}",
        ...
        .find(|session| session.foreground_command.is_some())
    {
        logging::info(
            TERMINAL_LOG_COMPONENT,
            format!(
                "PowerShell UI CWD probe v3 skipped: pid={:?} has foreground command {:?}",
    ...
    logging::info(
        TERMINAL_LOG_COMPONENT,
        format!(
            "PowerShell UI CWD probe v3: candidate_terminal_windows={} expected_shell_pids={:?}",
    ...
    if windows.is_empty() {
        logging::warn(
            TERMINAL_LOG_COMPONENT,
            "PowerShell UI CWD probe v3: no visible Windows Terminal/Cascadia top-level window was found",
        );
        return HashMap::new();
    }
    ...
    for (window_index, window) in windows.into_iter().enumerate() {
        ...
        logging::info(
            TERMINAL_LOG_COMPONENT,
            format!(
                "PowerShell UI CWD probe v3: focusing window_index={window_index} hwnd={:p} pid={}",
        ...
        if !focus_window(window.hwnd) {
            logging::warn(
                TERMINAL_LOG_COMPONENT,
                format!(
                    "PowerShell UI CWD probe v3: focus failed for window_index={window_index} hwnd={:p}",
                    window.hwnd
                ),
            );
            continue;
        ...
        logging::info(
            TERMINAL_LOG_COMPONENT,
            format!("PowerShell UI CWD probe v3: foreground acquired window_index={window_index}"),
        );
        ...
        let Some(original_probe) = probe_active_pane(window.hwnd, &nonce, step) else {
            logging::warn(
                TERMINAL_LOG_COMPONENT,
                format!(
                    "PowerShell UI CWD probe v3: original active pane did not answer in window_index={window_index}; aborting that window"
                ),
            );
            continue;
        };
        ...
        for tab_index in 0..MAX_TABS_PER_WINDOW {
            if unsafe { GetForegroundWindow() } != window.hwnd {
                ...
                logging::warn(
                    TERMINAL_LOG_COMPONENT,
                    format!(
                        "PowerShell UI CWD probe v3: foreground left Terminal before tab_index={tab_index}; aborting further input"
                    ),
                );
                break;
            }
            ...
            logging::info(
                TERMINAL_LOG_COMPONENT,
                format!(
                    "PowerShell UI CWD probe v3: selecting window_index={window_index} tab_index={tab_index}"
                ),
            );
            if !focus_terminal_tab(tab_index) {
                logging::warn(
                    TERMINAL_LOG_COMPONENT,
                    format!(
                        "PowerShell UI CWD probe v3: wt.exe focus-tab failed at tab_index={tab_index}"
                    ),
                );
                break;
            }
            ...
            if unsafe { GetForegroundWindow() } != window.hwnd {
                ...
                logging::warn(
                    TERMINAL_LOG_COMPONENT,
                    "PowerShell UI CWD probe v3: foreground changed after focus-tab; aborting further input",
                );
                break;
            }
            ...
            let Some(first_probe) = probe_active_pane(window.hwnd, &nonce, step) else {
                logging::warn(
                    TERMINAL_LOG_COMPONENT,
                    format!(
                        "PowerShell UI CWD probe v3: tab_index={tab_index} did not answer; stopping tab enumeration"
                    ),
                );
                break;
            };
            ...
            if !first_pids_by_tab.insert(first_pid) {
                logging::info(
                    TERMINAL_LOG_COMPONENT,
                    format!(
                        "PowerShell UI CWD probe v3: tab enumeration complete at tab_index={tab_index} repeated_pid={first_pid}"
                    ),
                );
                break;
            }
    ...
    logging::info(
        TERMINAL_LOG_COMPONENT,
        format!(
            "PowerShell UI CWD probe v3 complete: exact_results={} expected_sessions={} focus_interrupted={focus_interrupted}",
            results.len(), expected_pids.len(),
        ),
    );
    results
}
...
fn record_probe_result(
    ...
    if !expected_pids.contains(&pid) {
        logging::warn(
            TERMINAL_LOG_COMPONENT,
            format!(
                "PowerShell UI CWD probe v3: window_index={window_index} tab_index={tab_index:?} answered from unexpected pid={pid}; result ignored"
            ),
        );
        return;
    }
    ...
    logging::info(
        TERMINAL_LOG_COMPONENT,
        format!(
            "PowerShell UI CWD probe v3: window_index={window_index} tab_index={tab_index:?} pid={pid} exact_cwd={directory:?}"
        ),
    );
...
fn probe_active_pane(hwnd: Hwnd, nonce: &str, step: usize) -> Option<(u32, String)> {
    if unsafe { GetForegroundWindow() } != hwnd {
        logging::warn(
            TERMINAL_LOG_COMPONENT,
            "PowerShell UI CWD probe v3: SendInput blocked because Terminal is not foreground",
        );
        return None;
    }
    ...
    logging::info(
        TERMINAL_LOG_COMPONENT,
        format!(
            "PowerShell UI CWD probe v3: injecting cwd query step={step} utf16_units={}",
    ...
    if !send_unicode_text(&command) || unsafe { GetForegroundWindow() } != hwnd {
        logging::warn(
            TERMINAL_LOG_COMPONENT,
            "PowerShell UI CWD probe v3: Unicode SendInput failed or focus changed before Enter",
        );
        return None;
    }
    if !send_virtual_key(VK_RETURN) {
        logging::warn(
            TERMINAL_LOG_COMPONENT,
            "PowerShell UI CWD probe v3: Enter SendInput failed",
        );
        return None;
    }
    ...
    if result.is_none() {
        logging::warn(
            TERMINAL_LOG_COMPONENT,
            format!("PowerShell UI CWD probe v3: no result file appeared for step={step}"),
        );
    }
    result
}
...
fn focus_window(hwnd: Hwnd) -> bool {
    ...
    // Foreground activation is restricted by Windows. If the ordinary request
    // is rejected, temporarily join the input queues of the caller, current
    // foreground window and target Terminal thread. Do not call SetFocus on the
    // top-level Cascadia HWND; preserving Terminal's focused child is what makes
    // the injected text reach the active terminal control.
...
fn run_wt_command(args: &[&str]) -> bool {
        ...
        .spawn()
    {
        ...
        Err(error) => {
            logging::warn(
                TERMINAL_LOG_COMPONENT,
                format!("PowerShell UI CWD probe v3: failed to start wt.exe: {error}"),
            );
            return false;
        }
    };
    ...
    loop {
        match child.try_wait() {
            ...
            Ok(None) => {
                ...
                logging::warn(
                    TERMINAL_LOG_COMPONENT,
                    "PowerShell UI CWD probe v3: wt.exe command timed out",
                );
                return false;
            }
            Err(error) => {
                ...
                logging::warn(
                    TERMINAL_LOG_COMPONENT,
                    format!("PowerShell UI CWD probe v3: wt.exe wait failed: {error}"),
                );
                return false;
            }
        }
